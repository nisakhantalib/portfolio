window.portfolioProjects = {
  bangkit: {
    ...
    tags: [["LangGraph", "violet"], ["Python", "blue"], ["FastAPI", "green"], ["RAG", "gold"], ["Multimodal", "cyan"], ["Docker", "red"]],
    screenshot: "shots/alphaplus.jpg",
    summary:
      "A prototype exam-prep platform for Malaysian SPM students. The AI layer was rebuilt into a Python multi-agent service (LangGraph + FastAPI) with a supervisor coordinating tutor, quiz-generation, and marking agents over a hybrid-RAG-grounded curriculum, including multimodal input (photograph a handwritten answer to have it transcribed and marked).",
    introTitle: "Bangkit - Multi-Agent AI Learning Platform (Prototype)",
    overview:
      "A prototype learning platform for Malaysian SPM students, where a student can even photograph a handwritten answer and have it read and marked. The AI layer was rebuilt from direct LLM API calls into a Python multi-agent service (FastAPI + LangGraph): a supervisor agent classifies intent, extracts subject/chapter scope, and decomposes multi-step requests into an ordered plan, then routes to tutor, quiz-generation, and marking agents - each grounded in a hybrid RAG pipeline (BM25 keyword + vector search, rank-fused) over curriculum content. Tutor answers pass through a fact-verification node that cross-checks them against the retrieved context and drives one bounded self-correction when unsupported claims are found, then a presenter agent optionally attaches a diagram, table, or slides when a visual aids understanding. The Next.js frontend delegates AI work to the service via an opt-in, fall-back-safe integration.",
    features: [
      ["Multimodal input - mark a photographed answer", "Students photograph a handwritten SPM answer instead of retyping it. A vision model (Llama 4, via the same Groq stack) transcribes the image to text; the transcription is shown so the student can catch a misread; then the existing marker grades it. One transcribe node at the graph boundary - everything downstream (retrieval, marking, verification) is unchanged because it already operates on text. Vision is a hard routing constraint (never falls through to a text-only model) and the whole path fails open to typed text."],
    ...
    stack: [["AI service", "Python, FastAPI, LangGraph, Pydantic"], ["RAG", "Hybrid retrieval: BM25 (Okapi) + embeddings fused by reciprocal rank; metadata-filtered store; answer verification node; LangSmith"], ["LLM", "Groq SDK - Llama 3.3 70B (text) + Llama 4 (vision); multi-model fallback router with a hard vision constraint"], ["Frontend", "Next.js (App Router), React, Tailwind CSS"], ["Infra", "Docker, GitHub Actions CI/CD, Azure Container Apps (live), ACR"]],
    howItWorks: {
      intro:
